...
var createScene = function () {
    ...
    BABYLON.SceneLoader.ImportMesh("", baseUrl, fileName, scene, function (syncMesh) {
        ...
        //Env texture
        hdrTexture = new BABYLON.CubeTexture.CreateFromPrefilteredData(skyboxPath, scene);
...
function setBaseColor(mesh) {
    ...
    mesh._material._albedoTexture = new BABYLON.Texture(file + "_Base_Color.png", scene);
...
function setAll(mesh) {
    ...
    //Metallic
    mesh._material._metallicTexture = new BABYLON.Texture(file + "_Metallic.png", scene);
    //AO
    mesh._material._ambientTexture = new BABYLON.Texture(file + "_Mixed_AO.png", scene);
    mesh._material._ambientTexture = new BABYLON.Texture(file + "_Mixed_AO_A.png", scene);
}
...
function menuPlay() {
    for (var i = 1; i < currentMesh.length ; i++) {
        ...
        if (opened) {
            ...
            currentMesh[i]._material._ambientTexture = new BABYLON.Texture(file + "_Mixed_AO.png", scene);
        }
        else {
            scene.beginAnimation(currentMesh[i], 0, 2.7, false);
            currentMesh[i]._material._ambientTexture = new BABYLON.Texture(file + "_Mixed_AO_A.png", scene);
        }
    }
    opened = !opened;
}
